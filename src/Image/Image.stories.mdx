...
You can also make rounded images, using the `is-rounded` class on the `img` itself.
...
If you don't know the exact dimensions but know the **ratio** instead, you can use one of the **16 ratio modifiers**. Reference the `ratio` Component Prop to see all the values.
